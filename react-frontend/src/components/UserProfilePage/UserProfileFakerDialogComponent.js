...
import userProfileFakerFactory from "./userProfileFakerFactory";
...
const UserProfileFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userProfileFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userProfile").create(elem));
...
export default connect(mapState, mapDispatch)(UserProfileFakerDialogComponent);
